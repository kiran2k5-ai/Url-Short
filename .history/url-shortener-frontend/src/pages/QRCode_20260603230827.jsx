        ...
        <DashboardLayout>
            <div className="p-6 md:p-8">
                <div className="mb-8">
                    ...
                    <p className="text-sm md:text-base text-gray-600 mt-2">Download or share QR codes for your shortened URLs</p>
                </div>
                {loading ? (
                    ...
                    <div className="mt-6 md:mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
                        {urls.map((url) => (
                            <div key={url._id} className="bg-white rounded-2xl shadow-lg p-6 flex flex-col items-center hover:shadow-xl hover:scale-105 transition transform duration-300 border border-gray-100">
                                {qrCodes[url._id] ? (
                                    <>
                                        <div className="w-full mb-4 flex justify-center">
                                            <img src={qrCodes[url._id]} alt={`QR Code for ${url.shortCode}`} className="w-48 h-48 border-2 border-indigo-200 rounded-xl object-cover p-2 bg-white" />
                                        </div>
                                        <div className="w-full mb-4 text-center border-t border-b py-3">
                                            <p className="text-sm font-bold text-gray-900 bg-indigo-50 inline-block px-3 py-1 rounded-full">{url.shortCode}</p>
                                            <p className="text-xs text-gray-600 mt-2 truncate px-2 flex-1">{url.originalUrl}</p>
                                        </div>
                                        <div className="w-full flex gap-2">
                                            <button onClick={() => downloadQRCode(url._id, url.shortCode)} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 active:scale-95 transition text-sm hover:shadow-lg">
                                                <Download size={16} />
                                                Download
                                            </button>
                                            <button onClick={() => copyQRToClipboard(url.shortCode)} className="p-2.5 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 active:scale-95 transition hover:scale-110 font-medium" title="Copy short link">
                                                <Copy size={16} />
                                            </button>
                                        </div>
                                    </>
